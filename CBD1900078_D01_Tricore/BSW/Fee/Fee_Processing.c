...
#ifndef FEE_NEW_TRANSITION /* COV_FEE_COMPATIBILITY */
#   define FEE_NEW_TRANSITION(feeStack, topOfStack, successorTopOfStack, nextState, query0, query1, action0, action1)
#endif
 ...
 * \synchronous TRUE
 */
FEE_LOCAL_FUNC(void) Fee_Internal_LutCopyChunkSearchContext(Fee_ChunkSearchContextRefType target,Fee_LookUpTableContextRefType self);
 ...
 *********************************************************************************************************************/
/*!
 * \brief      Copies instance trailer page to correct poistion within the buffer
 * \details    Copies instance trailer page to correct poistion within the buffer
 * \param[in,out]  instBuf
 * \param[in]  length
 * \pre        -
 * \context    TASK
 * \reentrant  FALSE
 */
FEE_LOCAL_FUNC(void) Fee_InternalFillInstanceBufferTrailerPage(const Fee_IntBuffer_u8PtrType instBuf, uint32 length);
...
static CONST(struct Fee_StateDescrStruct, FEE_PRIVATE_CONST) Fee_StateDescrTable_at[] =
{
    ...
    /* FEE_STATE_CREATE_INSTANCE_BLOCK_COPY_WRITE_2 */
    {
        ...
        {
            FEE_STATE_ELSE {FEE_ACT_ID_FINISH_SUCCESSFUL_COPYING, FEE_ACT_ID_RETURN_FROM_SUB_FSM},
            FEE_NEXT_STATE(FEE_STATE_IDLE)
        }
    },
    ...
    {
        {
            ...
            {
                FEE_STATE_THEN {FEE_ACT_ID_EVALUATE_SECTOR_HEADERS, FEE_ACT_ID_LUT_INITIALIZATION},
                FEE_NEXT_STATE(FEE_STATE_STARTUP_READ_LUT_BLOCK)
            }
        },
        {
            FEE_STATE_ELSE {FEE_ACT_ID_EVALUATE_SECTOR_HEADERS, FEE_ACT_ID_RETURN_FROM_SUB_FSM},
            FEE_NEXT_STATE(FEE_STATE_IDLE)
        }
    },
#else
...
FEE_INTERN_FUNC(void) Fee_DoProcessing(Fee_GlobalsPtrType g)
{
    ...
    while(!g->InternalFlags_t.leaveMain) /* PRQA S 4558, 2872 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_4.1_Loop_Does_Terminate */
    {
...
#if(FEE_FORCE_SECTOR_SWITCH_API == STD_ON)
                && (!g->ApiFlags.forceSectorSwitch) /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
#endif
        ) /* COV_FEE_PREEMPTION */
        {
...
FEE_INLINE_FUNC(void) Fee_FsmProcess(Fee_FsmStackRefType fsmStack)
{
    ...
    if ((Fee_FsmQuery(CurrentExit_ptloc->ifChange_t.Queries_at) == FEE_TRUE)) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */ /* SBSW_FEE_FunctionPointerCall_FsmTable */
    {
        ...
        /* PRQA S 0432, 4423 6 */ /* MD_FEE_StateMachine_Tracer */
        FEE_NEW_TRANSITION(fsmStack->Stack_at, fsmStack->TopOfStack_u8, /* SBSW_FEE_Statetracer_New_Transition */
                           fsmStack->TopOfStack_u8, transition.NextState_t, /* SBSW_FEE_Statetracer_New_Transition */
                           CurrentExit_ptloc->ifChange_t.Queries_at[0],
                           CurrentExit_ptloc->ifChange_t.Queries_at[1],
                           transition.Actions_t.ActionHandler1_t, transition.Actions_t.ActionHandler2_t);
    }
    else
    {
        transition = CurrentExit_ptloc->elseChange_t;
        /* PRQA S 0432, 4423 5 */ /* MD_FEE_StateMachine_Tracer */
        FEE_NEW_TRANSITION(fsmStack->Stack_at, fsmStack->TopOfStack_u8, /* SBSW_FEE_Statetracer_New_Transition */
                           fsmStack->TopOfStack_u8, transition.NextState_t, /* SBSW_FEE_Statetracer_New_Transition */
                           STATETRACER_NO_QUERY, STATETRACER_NO_QUERY, transition.Actions_t.ActionHandler1_t,
                           transition.Actions_t.ActionHandler2_t);
    }
...
FEE_INLINE_FUNC(Fee_Bool) Fee_FsmQuery(Fee_StateQueryPtrType Queries_at)
{
    ...
    return (((Fee_QueryTable_ap[Queries_at[0u]]()) && (Fee_QueryTable_ap[Queries_at[1u]]()))); /* PRQA S 4403, 4558 */ /* MD_FEE_10.3_essentially_boolean_type, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* SBSW_FEE_FunctionPointerCall_FsmTable */ /* SBSW_FEE_FunctionPointerCall_FsmTable */
}
...
FEE_LOCAL_FUNC(void) Fee_FsmInitSubFsm(Fee_FsmStackRefType fsmStack, Fee_StateType InitState_t)
{
    ...
    /* PRQA S 0432, 4423 4 */ /* MD_FEE_StateMachine_Tracer */
    FEE_NEW_TRANSITION(fsmStack->Stack_at, fsmStack->TopOfStack_u8 - 1u, /* SBSW_FEE_Statetracer_New_Transition */
                       fsmStack->TopOfStack_u8, InitState_t, STATETRACER_NO_QUERY, STATETRACER_NO_QUERY, STATETRACER_NO_ACTION,
                       STATETRACER_NO_ACTION);
}
        ...
        case FEE_MNGMT_CONSISTENT:
            ...
            if(((((context->searchBlockId_t.Tag) ^ (blockId.Tag)) & (context->blockIdMask_t.Tag)) != 0u)
                    || ((Fee_ChunkInfo_isDataChunk(chunk)) /* SBSW_FEE_Func_passParamByReference */
                       && ((((context->searchBlockId_t.DataIndex) ^ (blockId.DataIndex)) & (context->blockIdMask_t.DataIndex)) != 0u))) /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
            {
 ...
 */
    ...
    FEE_LOCAL_FUNC(void) Fee_ActEvaluateWriteCycle(void)
    {
        ...
        const Fee_Bool wccErased = ((!Fee_ChunkInfo_isDataChunk(&instDescr->chunk_t)) /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* SBSW_FEE_ConstPointer_refParam */
...
{
    const Fee_ChunkSearchContextRefType context_pt = &Fee_Globals_t.FsmLocalVars_t.chunkSearchContext_t;
    Fee_SectorRefType searchSector;
    ...
    {
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
      if(!Fee_Internal_LutTryToStartLutSearch(searchSector)) /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* SBSW_FEE_Func_passParamByReference */
#endif
      {
...
FEE_LOCAL_FUNC(void) Fee_ActInitCreateInstanceFsm(void)
{
    ...
    if((!globals->FsmLocalVars_t.ChunkAllocContext_t.config_pt->CriticalData_bl) || /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
...
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
        if((blockConfig->PartitionIndex != (Fee_Partition_getId(g->CurrentProcessData.Partition_pt))) || (blockConfig->LookUpTableBlock_bl)) /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* COV_FEE_PART_CONFIG */ /* SBSW_FEE_PartitionConfig */
#else
        if((blockConfig->PartitionIndex != Fee_Partition_getId(g->CurrentProcessData.Partition_pt)))/* COV_FEE_PART_CONFIG */ /* SBSW_FEE_PartitionConfig */
...
FEE_LOCAL_FUNC(void) Fee_InternalInitLinkTableAllocationFsm(Fee_GlobalsPtrType g)
{
    ...
    if((datasets > 1u) && (Fee_Proc_isLinkTableChunkAllowed(&target->chunk_t))) /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* SBSW_FEE_Func_passParamByReference */
    {
...
FEE_LOCAL_FUNC(void) Fee_Proc_Write_SetupChunkAlloc(Fee_GlobalsPtrType g, Fee_ChunkAllocContextRefType ctx)
{
    if((Fee_Proc_isLinkTableChunkAllowed(&ctx->previousChunk_pt->chunk_t)) /* SBSW_FEE_ChunkAllocContext */
            && (ctx->config_pt->NumberOfDatasets_u8 > 1u)) /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */                 /* SBSW_FEE_ChunkAllocContext */
    {
        struct Fee_ChunkInfoStruct c;
...
FEE_INLINE_FUNC(void) Fee_Proc_Write_SetupAllocation(Fee_GlobalsPtrType g, Fee_ChunkAllocContextRefType ctx)
{
    ...
    if((Fee_ChunkInfo_isDataChunk(currChunk)) /* SBSW_FEE_ChunkAllocContext */
         && (ctx->previousChunk_pt->instance_t.index_t < Fee_ChunkInfo_GetInstanceCount(currChunk)) /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */  /* SBSW_FEE_ChunkAllocContext */
         && (Fee_Proc_isPayloadSizeCeiled(currChunk, ctx->config_pt)) /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */  /* SBSW_FEE_ChunkAllocContext */
...
FEE_LOCAL_FUNC(void) Fee_ActInitDataChunkAllocCopyFsm(void)
{
    ...
    {
        uint16 ceiledPayloadSize = Fee_Proc_CeilPayloadSize(source->chunk_t.Sector_pt,blockConfig->BlockPayloadSize_u16); /* SBSW_FEE_ConstPointer_refParam */
        if((blockConfig->BlockPayloadSize_u16 != payload) && (ceiledPayloadSize != payload)) /* SBSW_FEE_Func_passParamByReference */
        {
            source->instance_t.status_t = FEE_INSTANCE_STATUS_INVALID; /* SBSW_FEE_Func_writeObjectByReference */
        }
        payload = ceiledPayloadSize;
...
#if(FEE_GET_WRITE_CYCLE_API == STD_ON)
    /* PRQA S 4403, 4538, 4558 1 */ /* MD_FEE_10.3_essentially_boolean_type, MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
...
FEE_LOCAL_FUNC(void) Fee_ActInitializeBlockCopy(void)
{
    ...
    context_pt->sourceWrittenWithOldLayout = ( payload != Fee_Proc_CeilPayloadSize(source->chunk_t.Sector_pt,payload)); /* PRQA S 4403 */ /* MD_FEE_10.3_essentially_boolean_type */ /* SBSW_FEE_Func_writeObjectByReference */ /* SBSW_FEE_ConstPointer_refParam */
...
FEE_LOCAL_FUNC(void) Fee_ActPrepareBlockCopyRead(void)
{
    ...
    uint32 pageSize = Fee_Sector_alignTo(1,oldSector->cfg_pt->alignments_t.write_u8);
    if(context_pt->sourceWrittenWithOldLayout == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */
    {
        readLength = (context_pt->ByteCount_t > (FEE_INTERNAL_BUFFER_SIZE - pageSize)) ? (FEE_INTERNAL_BUFFER_SIZE - pageSize) : context_pt->ByteCount_t;
    }
    else
    {
        readLength = (context_pt->ByteCount_t > FEE_INTERNAL_BUFFER_SIZE) ? FEE_INTERNAL_BUFFER_SIZE : context_pt->ByteCount_t;
    }
...
FEE_LOCAL_FUNC(void) Fee_InternalDoBlockCopyWrite(Fee_GlobalsPtrType g)
{
    ...
    uint32 pageSize = Fee_Sector_alignTo(1,newSector_pt->cfg_pt->alignments_t.write_u8);
    uint32 spaceForTrailer = 0u;
    if(context_pt->sourceWrittenWithOldLayout == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */
    {
        writeLength = (context_pt->ByteCount_t > (FEE_INTERNAL_BUFFER_SIZE - pageSize)) ? (FEE_INTERNAL_BUFFER_SIZE - pageSize) : context_pt->ByteCount_t;
        if((context_pt->ByteCount_t - writeLength) == 0u)
        {
           Fee_InternalFillInstanceBufferTrailerPage(&context_pt->BufferBytes[writeLength],pageSize); /* SBSW_FEE_InternalDataBuffer_passedTo */
           spaceForTrailer = pageSize;
        }
    }
    else
    {
    ...
    Fee_InternalEvalFlsRetVal(Fee_Sector_writeData(newSector_pt, /* SBSW_FEE_Partitions_OldestNewestSectorRef */
                         context_pt->targetOffset_t,
                         context_pt->BufferBytes, writeLength + spaceForTrailer));
...
FEE_LOCAL_FUNC(void) Fee_InternalDoWriteStreamLast(Fee_GlobalsPtrType g)
{
    const Fee_InstanceWriteContextRefType context = &g->FsmLocalVars_t.instanceWriteContext_t;
    uint32 pageSize = Fee_Sector_alignTo(1u,context->sector_pt->cfg_pt->alignments_t.write_u8);
    Fee_InternalFillInstanceBufferTrailerPage(context->BufferBytes,pageSize); /* SBSW_FEE_InternalDataBuffer_passedTo */ /* SBSW_FEE_ConstPointer_refParam */
    Fee_InternalEvalFlsRetVal(Fee_Sector_writeData(context->sector_pt, context->targetOffset_t, context->BufferBytes, pageSize)); /* SBSW_FEE_ConstPointer_refParam */
}
...
FEE_LOCAL_FUNC(void) Fee_ActReturnFromSubFsm(void)
{
    ...
    /* PRQA S 0432, 4423 6 */ /* MD_FEE_StateMachine_Tracer */
    FEE_NEW_TRANSITION(Fee_Globals_t.FsmStack_t.Stack_at, /* SBSW_FEE_Statetracer_New_Transition */
                       Fee_Globals_t.FsmStack_t.TopOfStack_u8 + 1u, Fee_Globals_t.FsmStack_t.TopOfStack_u8,
                       Fee_Globals_t.FsmStack_t.Stack_at[Fee_Globals_t.FsmStack_t.TopOfStack_u8], STATETRACER_NO_QUERY,
                       STATETRACER_NO_QUERY, STATETRACER_NO_ACTION, STATETRACER_NO_ACTION);
}
...
FEE_LOCAL_FUNC(void) Fee_Internal_ValidateChunkLinkInContext(const Fee_ChunkSearchContextRefType context_pt)
{
  ...
  default: /*FEE_MNGMT_CONSISTENT*/
    ...
    if (((context_pt->super_t.searchBlockId_t.DataIndex) != 0u) && /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
       (Fee_ChunkInfo_isInitialLinkTable(currentChunk_pt)) && /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* SBSW_FEE_Func_passParamByReference */
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_QryBlockPayloadUpdated(void)
{
    ...
    if((Fee_ChunkInfo_isDataChunk(chunk_pt)) && (blockCfg_pt != NULL_PTR)) /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* SBSW_FEE_ChunkSearchContext */
    {
...
FEE_LOCAL_FUNC(void) Fee_InternalFillInstanceBufferTrailerPage(const Fee_IntBuffer_u8PtrType instBuf, uint32 length)
{
    Fee_InternalFillInstanceBufferWithValue(instBuf,(uint8)FEE_INSTANCE_STATUS_OK, length); /* SBSW_FEE_InternalDataBuffer_writeAccess */
}
...
FEE_LOCAL_FUNC(Fee_InstanceStatusType) Fee_InternalEvalInstHdrStatus(Fee_InstanceSearchContextConstRefType context_pt, MemIf_JobResultType flsResult)
{
    ...
    {
        uint32 u_index = ((Fee_Sector_alignWrite(sector, FEE_INSTANCE_HEADER_SIZE) - 1u) >> 2u); /* SBSW_FEE_Chunks_Sector_ref */
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_InternalScheduleProcess(Fee_GlobalsPtrType g)
{
    ...
    if((reqJob != FEE_JOB_NONE) /* PRQA S 1881 */ /* MD_FEE_10.4_compare_enums */
            ...
            &&        (!g->InternalFlags_t.forceSectorSwitch) /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
#endif /* ECO_IGNORE_BLOCK_END */
#if(FEE_LOOKUPTABLE_MODE == STD_ON)
    /* An ongoing LookUpTable synchronization shall not be interrupted by a "normal" job */
            &&        (!g->InternalFlags_t.syncLookUpTable) /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
#endif
      )
    {
        ...
        if((reqJob <= FEE_JOB_WRITE) && /* PRQA S 4558, 1881, 1880 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_10.4_compare_enums, MD_FEE_10.4_compare_enums */
  ...
  else if (g->InternalFlags_t.syncLookUpTable == FEE_TRUE) /* PRQA S 1881 */  /* MD_FEE_10.4_compare_FeeBool_to_enum */
  {
    ...
    else
    {
      ...
      if (!processStarted) /* PRQA S 4538 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
      {
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_InternalInitSectorSwitchFsm(Fee_GlobalsPtrType globals, Fee_PartitionRefType partition)
{
    ...
    Fee_Bool processStarted = FEE_FALSE;  /* PRQA S 4423 */ /* MD_FEE_10.3_essentially_enum_type */
    Fee_Bool ssAllowed = (Fee_Partition_isWriteable(partition)) && (ssBlocked == FEE_FALSE); /* PRQA S 4403, 1881, 4558 */ /* MD_FEE_10.3_essentially_boolean_type, MD_FEE_10.4_compare_FeeBool_to_enum, MD_FEE_10.1_Essentially_boolean_in_logical_expressions */ /* SBSW_FEE_ConstPointer_refParam */
    ...
    if((globals->InternalFlags_t.syncLookUpTable == TRUE) && (!ssAllowed)) /* PRQA S 4538 */ /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
    {
...
{
  const Fee_GlobalsPtrType globals = &Fee_Globals_t;
  Fee_LookUpTableSyncDataRefType LookUpTableSyncContextPtr = &globals->LookUpTableSyncData_t;
  const Fee_PartitionRefType jobPartition_pt = globals->CurrentProcessData.Partition_pt;
...
FEE_LOCAL_FUNC(void) Fee_ActLutValidateBlockContent(void)
{
  ...
  Fee_ChunkInfoRefType chunk = &Fee_Globals_t.CurrentDataset_t.chunk_t;
  Fee_LookUpTableSyncDataRefType syncContext = &Fee_Globals_t.LookUpTableSyncData_t;
  if (!chunk->isPseudoChunk_bl) /* PRQA S 4558 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions */
  {
    Fee_PartitionConstRefType jobPartition_pt = Fee_Globals_t.CurrentProcessData.Partition_pt;
...
FEE_LOCAL_FUNC(void) Fee_ActLutFinishBlockSynchronization(void)
{
  ...
  {
    Fee_PartitionRefType part_pt = Fee_Partition_getFromConfigById(Fee_Globals_t.Config_pt,syncContext->LookUpTablePartitionIndex); /* SBSW_FEE_PartitionConfig */
...
FEE_LOCAL_FUNC(Fee_Bool) Fee_Internal_LutTryToStartLutSearch(Fee_SectorRefType searchSector)
{
  ...
  Fee_ProcessDataRefType processData_pt = &Fee_Globals_t.CurrentProcessData;
...
FEE_LOCAL_FUNC(void) Fee_Internal_LutCopyChunkSearchContext(Fee_ChunkSearchContextRefType target,Fee_LookUpTableContextRefType self)
{
  Fee_ChunkSearchContextRefType lutSearchContext_pt = &self->chunkSearchContext_t;
...
FEE_LOCAL_FUNC(void) Fee_ActFinishSuccessfulCopying(void)
{
  ...
  Fee_ChunkInfoRefType chunk = &Fee_Globals_t.FsmLocalVars_t.instanceCopyContext_t.targetChunk_t;
  Fee_LookUpTableContextRefType lutContext_pt = &Fee_Globals_t.LookUpTableContext_t;
...
FEE_INLINE_FUNC(Fee_Bool) Fee_Proc_isLinkTableChunkAllowed(Fee_ChunkInfoConstRefType chunk)
{
    ...
    return ((Fee_ChunkInfo_isInitialLinkTable(chunk)) /* SBSW_FEE_PointerWrite_refParam */
            && (Fee_ChunkInfo_isLinkAllowed(chunk))); /* PRQA S 4558, 3415 */  /* MD_FEE_10.1_Essentially_boolean_in_logical_expressions, MD_FEE_13.5_queries */ /* SBSW_FEE_PointerWrite_refParam */
}

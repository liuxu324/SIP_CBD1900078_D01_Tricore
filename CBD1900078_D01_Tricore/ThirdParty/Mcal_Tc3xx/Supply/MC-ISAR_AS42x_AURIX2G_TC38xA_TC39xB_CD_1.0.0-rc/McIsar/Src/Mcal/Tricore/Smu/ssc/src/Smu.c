...
**                      Includes                                              **
*******************************************************************************/
/*[cover parentID={825C220C-45ED-44f2-9CDA-CE9D43F07B83}] [/cover]*/
#include "McalLib.h"
#include "Smu.h"
#include "SchM_Smu.h"
                               ...
                               const Smu_FSPActionType FSPAction)
{
    ...
    else if(AlarmGroup == SMU_ALARM_GROUP20)
    {
        ...
        [/cover] */
        SMU_LIB_RUNTIME_WRITESAFETYENDINITPROTREG( \
        &MODULE_PMS.AGFSP_STDBY0,((SMU_STDBY_BIT_PROT| (FSPAction<<AlarmPos)) \
        |PMS_AGFSP_STDBY0.U));
        RetVal = E_OK;
    }
    else 
...
)
{
    ...
    #endif
    {
        ...
        if(MCAL_MASTER_COREID == CurrentCoreID)
        {
            ...
            [/cover] */
            /* [cover parentID={20BE67A9-790F-4e0f-961F-F823089B98E1}][/cover] */
            ...
            #endif
            {
                ...
                /*Configure FSP registers of Smu_core as per the configuration*/
                SMU_LIB_INIT_DEINIT_WRITESAFETYENDINITPROTREG \
                (&MODULE_SMU.FSP,ConfigPtr->FSPCfg);

                /*Configure alam global configuration of Smu_core register 
...
Std_ReturnType Smu_DeInit(void)
{
    ...
    if(MCAL_MASTER_COREID == CurrentCoreID)
    {
        ...
        #endif
        {
            ...
            /* Configure FSP register of Smu_core to its reset value */
            SMU_LIB_INIT_DEINIT_WRITESAFETYENDINITPROTREG( \
            &MODULE_SMU.FSP, SMU_FSP_RESET_VAL);
...
)
{
    ...
    #endif
    {
        ...
        do
        {
            ...
            {
                /* Read Alarm status */
                /* [cover parentID={4033D435-5170-4567-9947-98E5C7A55820}]
...
)
{
    ...
    #endif
    {
        ...
        [/cover] */
        #if(SMU_STANDBY_SWITCH == STD_ON)
        /* [cover parentID={FE9C5A57-9C86-4a94-9BA3-41A566721C03}]
        Check if the alarm group is SMU Alarm Group 20
        [/cover] */
        if(AlarmGroup == SMU_ALARM_GROUP20)
        {
            /* [cover parentID={CE9FE261-B5BA-48c1-B677-5064739CF768}]
            Read the alarm status from Alarm group 20
            [/cover] */
            *AlarmStatus =(uint32)(PMS_AG_STDBY0.U);
            RetVal = E_OK;
        }
        /* [cover parentID={2397E765-DA4E-480d-B287-0F01704B89B5}]
        Check if alarm group is SMU Alarm Group 21
        [/cover] */
        else if(AlarmGroup == SMU_ALARM_GROUP21)
        {
            /* [cover parentID={754A69B3-F3D4-4598-A0EB-B55E29C32B50}]
            Read the alarm status from Alarm group 21
            [/cover] */
        ...
        #endif  
        {
            ...
            #endif
            {
                /* [cover parentID={9854A637-8137-4430-BFA5-D34F09967D05}]
                Read the alarm status of alarm group in core domain
                [/cover] */
                /*MISRA2012_RULE_11_3_JUSTIFICATION: Conversion between 
                pointers of different object types due to SFR access. */
...
Std_ReturnType Smu_ReleaseFSP(void)
{
    ...
    /* [cover parentID={D1FB1959-3FB3-4123-92D8-226B551E6129}][/cover] */
    Std_ReturnType RetVal = E_NOT_OK;
    Smu_CoreStateType SmuState;
    uint32 SmuEFRST;
...
Std_ReturnType Smu_SetupErrorPin(void)
{
    ...
    #endif
    {
        ...
        /* [cover parentID={6731DC2E-21D7-4542-9CCB-7446602BACC6}]
...
Std_ReturnType Smu_ReleaseErrorPin(void)
{
    ...
    #endif
    {
        ...
        /* [cover parentID={603CA49C-F59A-4ae4-9B5A-85BB8EF98993}]
...
**                                                                            **
** Syntax           : Smu_StateType Smu_ActivatePES (void)                    **
**                                                                            **
...
**                                                                            **
** Syntax           : Smu_StateType Smu_CoreAliveTest (void)                  **
**                                                                            **
...
)
{
    ...
    if(AlarmExecStatus != NULL_PTR)
    {
        ...
        else
        {
            ...
            Report SMU_SID_GET_ALARM_EXEC_STS
            [/cover] */
...
)
{
    ...
    if(RetVal == E_OK)
    {
        /* [cover parentID={5ECCEA08-F23B-4496-819C-891182683F67}]
        Check if requested Alarm execution Status is reserved
        [/cover] */
        if(AlarmExecStatusReq > SMU_ALMEXEC_MSK)
        {
            /* [cover parentID={65964888-45B9-4559-8873-6C6DD6F26CD2}]
            Report error SMU_E_INVALID_EXECUTION_STATUS
            [/cover] */
            Smu_lReportError(SMU_SID_CLEAR_ALARM_EXEC_STS, \
            SMU_E_INVALID_EXECUTION_STATUS);
            /* [cover parentID={DF0C47B1-E964-474d-A62C-126E2C2D7BA5}]
            Return Result as E_NOT_OK
            [/cover] */
            RetVal = E_NOT_OK;
        }
        else
        {
            ...
            if(AlarmRes == SMU_ALARM_INVALID)
            {
                ...
                Smu_lReportError(SMU_SID_CLEAR_ALARM_EXEC_STS, \
                SMU_E_INVALID_EXECUTION_STATUS);
                /* [cover parentID={3C2DEB2F-C41A-45cc-A6E6-E3BB455DC66B}]
                Set result as E_NOT_OK
                [/cover] */
                RetVal = E_NOT_OK;
            }

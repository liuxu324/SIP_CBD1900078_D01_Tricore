...
#if ( LINIF_SW_MAJOR_VERSION != (6u) ) /* COV_LINIF_CONFIG XF */
...
#if ( LINIF_TPS_MAJOR_VERSION != (6u) ) /* COV_LINIF_CONFIG XF */
...
#if ( LINIF_CBK_MAJOR_VERSION != (6u) ) /* COV_LINIF_CONFIG XF */
...
#if ( LINIF_TP_SUPPORTED == STD_ON )
# if ( LINTP_TPS_MAJOR_VERSION != (6u) ) /* COV_LINIF_CONFIG XF */
...
)
{
  ...
  else
  {
    ...
    if ( schedTabDataPtrTmp->LinIf_CurrentSlotDelay == (LinIf_ScheduleTableTickType)0u )
    {
    ...
    {
       if ( ( LinIf_IsScheduleChangeBeforeSlotEndOfChannelConfig(LinIf_LinIfChannel) ) &&
            ( schedTabDataPtrTmp->LinIf_RequestedTable != LinIf_Schedule_Invalid ) &&
            ( LinIf_GetChannelData(LinIf_LinIfChannel).LinIf_FrameMaxDelay == (LinIf_ScheduleTableTickType)0u ) ) /* COV_LINIF_SCHEDULECHANGE_POINT_1 */
       {
    ...
    if ( retVal == LinIf_SchdlHdlrRet_NewMessage )
    {
      ...
      else
      {
        ...
        else
        {
          ...
          *LinIf_FrameMaxDelayPtr                     = (LinIf_ScheduleTableTickType)LinIf_GetFrameLengthDelayOfFrameLengthDelayList( /* SBSW_LINIF_PASSED_PTR_ACCESS_IN_STATIC_FUNC */
                                                         LinIf_GetFrameLengthDelayListStartIdxOfChannelConfig(LinIf_LinIfChannel) + ((uint16)frameLengthTmp - 1u));
#endif
...
)
{
  ...
  while ( sporadicFrameTmp < LinIf_GetUncFrameCountOfSporadicFrameInfo(LinIf_GetFixedFrameSduListIdxOfFrameList(LinIf_ChannelDataPtr->LinIf_FrameHandle)) )
  {
    ...
    if ( (LinIf_GetSporTransmitFlags(byteMskTmp) & bitMskTmp) != (LinIf_SporTransmitFlagsType)0u) /* COV_LINIF_NO_SPORFRAME_ON_TESTCHANNEL */
...
#if ( LINIF_J2602_FRAME_TOLERANCE_SUPPORT == STD_ON )
      LinIf_ChannelDataPtr->LinIf_FrameMaxDelay            = (LinIf_ScheduleTableTickType)LinIf_GetSleepFrameLengthDelayInTicksOfChannelConfig(LinIf_LinIfChannel); /* SBSW_LINIF_CHANNEL_DATA_PTR */
#else
      LinIf_ChannelDataPtr->LinIf_FrameMaxDelay            =  LinIf_GetFrameLengthDelayOfFrameLengthDelayList( /* SBSW_LINIF_CHANNEL_DATA_PTR */
                                                              LinIf_GetFrameLengthDelayListStartIdxOfChannelConfig(LinIf_LinIfChannel) + 7u);
#endif
    }
...
(
  CONSTP2VAR(LinIf_ChannelDataType, AUTOMATIC, LINIF_VAR_NOINIT)            LinIf_ChannelDataPtr,
  CONSTP2VAR(Lin_PduType, AUTOMATIC, LINIF_VAR_NOINIT)                      LinIf_LinPduPtr,
  P2VAR(LinIf_FrameListIdxOfEntryType, AUTOMATIC, LINIF_VAR_NOINIT)         LinIf_FrameHandlePtr /* PRQA S 3673 */ /* MD_LINIF_Rule8.13_3673 */
#if ( LINIF_TP_SUPPORTED == STD_ON )
  ,CONSTP2VAR(LinTp_ControlType, AUTOMATIC, LINIF_VAR_NOINIT)               LinTp_CtrlPtr
#endif
)
...
# endif
            {
              sporTxFlags = (uint8)(LinIf_GetSporTransmitFlags(byteMskTmp) & (uint8)(~bitMskTmp));
              LinIf_SetSporTransmitFlags(byteMskTmp, sporTxFlags); /* SBSW_LINIF_SPOR_TXFLAGS_ACCESS */
            }
          }
    ...
    if (LinIf_ChannelDataPtr->LinIf_MirroringActivationState == TRUE)
    {
      /* store bus mirroring event data for later reporting and set pending flag */
      LinIf_ChannelDataPtr->LinIf_MirroringEventPending    = TRUE; /* SBSW_LINIF_CHANNEL_DATA_PTR */
      LinIf_ChannelDataPtr->LinIf_MirroringPid             = LinIf_GetPidOfFrameList(LinIf_ChannelDataPtr->LinIf_FrameHandle); /* SBSW_LINIF_CHANNEL_DATA_PTR */
      LinIf_ChannelDataPtr->LinIf_MirroringLinStatus       = LinIf_LinStatus; /* SBSW_LINIF_CHANNEL_DATA_PTR */
      ...
      if ( LinIf_LinStatus == (Lin_StatusType)LIN_TX_OK )
      {
        ...
        {
          LinIf_ChannelDataPtr->LinIf_RxMirrorData[indexTmp & LINIF_LINPDU_SIZE_MASK] = LinIf_ChannelDataPtr->LinIf_TxData[indexTmp & LINIF_LINPDU_SIZE_MASK]; /* SBSW_LINIF_RX_BUFFER_ARRAY */
        }
  ...
  else if ( LinIf_GetDirectionOfFrameList(LinIf_ChannelDataPtr->LinIf_FrameHandle) == LIN_SLAVE_RESPONSE ) /* COV_LINIF_MISRA_ELSEBRANCH */
  {
    ...
    if ( LinIf_LinStatus == (Lin_StatusType)LIN_RX_OK )
    {
      ...
      { /* copy Lin receive data to LinIf buffer for data consistency */
        LinIf_ChannelDataPtr->LinIf_RxMirrorData[indexTmp & LINIF_LINPDU_SIZE_MASK] = LinIf_LinSduPtr[indexTmp & LINIF_LINPDU_SIZE_MASK]; /* SBSW_LINIF_RX_BUFFER_ARRAY */
      }
    }
...
# endif
      ...
      for (indexTmp = 0; indexTmp < LinIf_GetLengthOfFrameList(LinIf_ChannelDataPtr->LinIf_FrameHandle); indexTmp++)
      { /* copy Lin receive data to LinIf buffer for data consistency */
        LinIf_ChannelDataPtr->LinIf_RxMirrorData[indexTmp]       = LinIf_LinSduPtr[indexTmp]; /* SBSW_LINIF_CHANNEL_DATA_PTR */
      }
    }
  }
  /* #40 Otherwise: */
...
LINIF_LOCAL_INLINE FUNC(void, LINIF_CODE) LinIf_MessageProcessing_SRF
(
  CONSTP2VAR(LinIf_ChannelDataType, AUTOMATIC, LINIF_VAR_NOINIT)  LinIf_ChannelDataPtr, /* PRQA S 3206 */ /* MD_LINIF_Rule2.7_3206 */
  P2CONST(uint8, AUTOMATIC, LINIF_APPL_VAR)                       LinIf_LinSduPtr, /* PRQA S 3206 */ /* MD_LINIF_Rule2.7_3206 */
  VAR(NetworkHandleType, AUTOMATIC)                               LinIf_LinIfChannel, /* PRQA S 3206 */ /* MD_LINIF_Rule2.7_3206 */
  VAR(Lin_StatusType, AUTOMATIC)                                  LinIf_LinStatus /* PRQA S 3206 */ /* MD_LINIF_Rule2.7_3206 */
)
{
  uint8                                                           errorId;
  ...
  LINIF_DUMMY_STATEMENT_CONST(LinIf_ChannelDataPtr); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
  LINIF_DUMMY_STATEMENT(LinIf_LinSduPtr); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
  LINIF_DUMMY_STATEMENT(LinIf_LinIfChannel); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
  LINIF_DUMMY_STATEMENT(LinIf_LinStatus); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
...
#if ( LINIF_BUSMIRRORING == STD_ON )
  channelDataPtrTmp->LinIf_MirroringActivationState               = FALSE; /* SBSW_LINIF_CHANNEL_DATA_PTR */
  channelDataPtrTmp->LinIf_MirroringEventPending                  = FALSE; /* SBSW_LINIF_CHANNEL_DATA_PTR */
  channelDataPtrTmp->LinIf_MirroringPid                           = 0x40u; /* invalid PID - dummy value which is always overwritten before being used */ /* SBSW_LINIF_CHANNEL_DATA_PTR */
  channelDataPtrTmp->LinIf_MirroringLinStatus                     = LIN_CH_SLEEP; /*  dummy value which is always overwritten before being used */ /* SBSW_LINIF_CHANNEL_DATA_PTR */
#endif
...
)
{
  ...
  if ( LinIf_ChannelDataPtr->LinIf_MirroringEventPending == TRUE )
  {
    /* #20 Setup frame data and parameters to be reported */
    if ( (LinIf_ChannelDataPtr->LinIf_MirroringLinStatus == LIN_RX_OK) || (LinIf_ChannelDataPtr->LinIf_MirroringLinStatus == LIN_TX_OK) )
    {
      pduInfo.SduLength = LinIf_ChannelDataPtr->LinIf_PduRNotificationPduLength;
      pduInfo.SduDataPtr = &(LinIf_ChannelDataPtr->LinIf_RxMirrorData[0]);
    }
    else
    {
    ...
    Mirror_ReportLinFrame(LinIf_GetSystemChannelIndexOfChannelIndTable(LinIf_LinIfChannel), LinIf_ChannelDataPtr->LinIf_MirroringPid, /* SBSW_LINIF_FCT_CALL_EXTERNAL_WITH_PTR_STACK_VAR */
                          &pduInfo, LinIf_ChannelDataPtr->LinIf_MirroringLinStatus);

    /* clear pending mirror notification */
    LinIf_ChannelDataPtr->LinIf_MirroringEventPending = FALSE; /* SBSW_LINIF_CHANNEL_DATA_PTR */
  }
}
#endif
...
#if ( LINIF_SPORADIC_FRM_SUPPORTED == STD_ON )
      if (LinIf_GetSporUncFrmTotalNumber() != 0u) /* COV_LINIF_NO_SPORFRAME_ON_TESTCHANNEL */ /* PRQA S 2741 */ /* MD_LINIF_Rule14.3_2741_Spor */
...
# endif
...
#endif
  {
    ...
    for (linIfChannelTmp = 0x00u; linIfChannelTmp < LinIf_GetSizeOfChannelData(); linIfChannelTmp++)
    {
      ...
      if ( LinIf_GetChannelData(linIfChannelTmp).LinIf_ChannelFuncCallCycleCounter == 0u ) /* COV_LINIF_FUNC_CALL_CYCLE_CONFIG */
      {
  ...
  for (linIfChannelTmp = 0x00u; linIfChannelTmp < LinIf_GetSizeOfChannelData(); linIfChannelTmp++)
  {
    ...
    **********************************************************/
    ...
    { /* report received frame to PduR with enabled interrupts */
      ...
      pduInfo.SduDataPtr = &(channelDataPtrTmp->LinIf_RxMirrorData[0]);
#if ( LINIF_PDUR_ONLY_UL_OPTIMIZATION == STD_OFF )
    ...
    **********************************************************/
    ...
    LinIf_MainFunctionPostProcessing_Mirror(channelDataPtrTmp, linIfChannelTmp); /* SBSW_LINIF_FCT_CALL_PTR_HANDOVER_OR_LOCAL_OR_CSL */
#endif
...
# endif
    {
      ...
      channelDataPtrTmp->LinIf_MirroringActivationState = MirroringActive; /* SBSW_LINIF_CHANNEL_DATA_PTR_TMP */
      /* clear a possibly pending mirror. If mirroring is disabled, no further events shall be reported.
         If mirroring is going to be enabled, then the event pending flag is never set and following line has no effect */
      channelDataPtrTmp->LinIf_MirroringEventPending    = FALSE; /* SBSW_LINIF_CHANNEL_DATA_PTR_TMP */
...
# if ( LINIF_DEV_ERROR_REPORT == STD_ON )
  if ( errorId != LINIF_E_NO_ERROR )
  {
    (void)Det_ReportError(LINIF_MODULE_ID, LINIF_INSTANCE_ID_DET, LINIF_SID_BUSMIRRORING, errorId);
  }
# else
  LINIF_DUMMY_STATEMENT(errorId); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
# endif
...
)
{
...
#endif
  {
    ...
    else
    {
      ...
      else if ( ( Schedule > (LinIf_SchHandleType)((LinIf_SchHandleType)LinIf_GetScheduleTableListInd(LinIf_GetScheduleTableListIndEndIdxOfChannelConfig(linIfChannelTmp) - 1u) -
                                                    LinIf_GetScheduleTableListInd(LinIf_GetScheduleTableListIndStartIdxOfChannelConfig(linIfChannelTmp))) )
#if (LINIF_INVALIDHNDOFSCHEDULETABLELIST == STD_ON)
             || ( LinIf_IsInvalidHndOfScheduleTableList( LinIf_GetScheduleTableListInd(LinIf_GetScheduleTableListIndStartIdxOfChannelConfig(linIfChannelTmp)) + Schedule) )
#endif
...
# endif
    ...
    else
    {
      ...
      else
      {
        ...
        if ( channelDataPtrTmp->LinIf_SchedTabData.LinIf_CurrentSlot == 0u)
        {
        ...
        {
          ScheduleInfo->CurrentSlot       = (uint8)(channelDataPtrTmp->LinIf_SchedTabData.LinIf_CurrentSlot - 1u); /* SBSW_LINIF_SCHEDULEINFO_PTR */
        }
...
# if ( LINTP_FORWARD_RESPONSEPENDING_TO_PDUR == STD_ON )
      if (LinTp_CtrlPtr->LinTp_RespPendingCounter == 0u)
# endif
...
)
{
  ...
  if ( LinTp_GetNADOfRxNSdu(LinTp_CtrlPtr->LinTp_Curr_Rx_Id) == LinTp_LinSduPtr[0] ) /* COV_LINTP_STRICTNADCHECK_UNREACHEDCODE TX */
  {
    ...
    { /* valid FF length */
      ...
      LinTp_CtrlPtr->LinTp_Timer                       = LinTp_GetNcrOfRxNSdu(LinTp_CtrlPtr->LinTp_Curr_Rx_Id); /* SBSW_LINIF_TP_CTRL_PTR */
      LinTp_CtrlPtr->LinTp_TimerReload                 = LinTp_GetNcrOfRxNSdu(LinTp_CtrlPtr->LinTp_Curr_Rx_Id); /* SBSW_LINIF_TP_CTRL_PTR */
      LinTp_CtrlPtr->LinTp_RemainingLength             = (PduLengthType)( (((uint16)LinTp_LinSduPtr[1] & 0x0Fu) << 8u) | (LinTp_LinSduPtr[2]) ); /* SBSW_LINIF_TP_CTRL_PTR */
...
)
{
  ...
  { /* CF awaited, note that NAD is already checked */
    ...
    if ( LinTp_CtrlPtr->LinTp_SN == (uint8)(LinTp_LinSduPtr[1] & 0x0Fu) )
    {
      ...
      LinTp_CtrlPtr->LinTp_SN                       = (uint8)((LinTp_CtrlPtr->LinTp_SN + 1u) & 0x0Fu); /* SBSW_LINIF_TP_CTRL_PTR */
...
)
{
  ...
  if ( ( !(LinTp_IsLinTp_StrictNADCheckOfChannelConfig(linTpChannelTmp)) ) &&
...
# endif
  ...
  if ( ((tpCtrlTmp->LinTp_Ch_State & LINTP_CHANNEL_TX_MSK ) == 0u) && (tpCtrlTmp->LinTp_NAD_Requested == LinTp_LinSduPtr[0]) )
  {
...
(
  CONSTP2VAR(LinTp_ControlType, AUTOMATIC, LINIF_VAR_NOINIT)      LinTp_CtrlPtr,
  CONSTP2VAR(LinIf_ChannelDataType, AUTOMATIC, LINIF_VAR_NOINIT)  LinTp_IfChannelDataPtr, /* PRQA S 3206 */ /* MD_LINIF_Rule2.7_3206 */
  CONSTP2VAR(Lin_PduType, AUTOMATIC, LINIF_VAR_NOINIT)            LinTp_LinPduPtr
)
{
  /* ----- Local Variables ---------------------------------------------- */
...
# endif
  ...
  /* ----- Implementation ----------------------------------------------- */
  LINIF_DUMMY_STATEMENT_CONST(LinTp_IfChannelDataPtr); /* PRQA S 1338, 2983, 3112 */ /* MD_MSR_DummyStmt */
...
#  endif
    ...
    if (bufferReqRetVal == (BufReq_ReturnType)BUFREQ_OK)
    {
      ...
      uint8 i;
      for (i = (uint8)(LinTp_LinPduPtr->SduPtr[1] + 2u); i < LINIF_LINPDU_SIZE; i++)
      {
...
# if ( LINTP_FUNCTIONAL_REQUEST_SUPPORTED == STD_ON )
    ...
    if ( (LinTp_GetNADOfTxNSdu(LinTpTxSduId) == 0x7Eu) && (LinTpTxInfoPtr->SduLength <= 6u) )
    {
...
# endif
      ...
      if ( tpCtrlTmp->LinTp_Ch_State == LINTP_CHANNEL_IDLE )
      {
        ...
        {
          /* SF request */
          tpCtrlTmp->LinTp_Tx_Rx_Buffer[1]        = (uint8)(LinTpTxInfoPtr->SduLength); /* SBSW_LINIF_TP_TRANSMIT_CTRL_PTR */
        ...
        else
        {
          ...
          tpCtrlTmp->LinTp_Tx_Rx_Buffer[1]        = (uint8)(((LinTpTxInfoPtr->SduLength & 0x0F00u) >> 8u) | LinTp_FrameType_FF); /* SBSW_LINIF_TP_TRANSMIT_CTRL_PTR */
          /* set lower data length byte */
          tpCtrlTmp->LinTp_Tx_Rx_Buffer[2]        = (uint8)  (LinTpTxInfoPtr->SduLength & 0x00FFu); /* SBSW_LINIF_TP_TRANSMIT_CTRL_PTR */
...
\ID SBSW_LINIF_RX_BUFFER_ARRAY
    \DESCRIPTION Array access to local Rx buffer LinIf_RxMirrorData with size LINIF_LINPDU_SIZE via index variable indexTmp.
    \COUNTERMEASURE \R The index cannot exceed the size of the array because the index is limited to range from 0

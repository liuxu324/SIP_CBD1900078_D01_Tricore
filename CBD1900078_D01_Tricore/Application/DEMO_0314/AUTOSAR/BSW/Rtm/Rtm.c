...
# if !defined Rtm_Hook_StartCalcCpuLoadThreshold
#  define Rtm_Hook_StartCalcCpuLoadThreshold()
# endif
# if !defined Rtm_Hook_StopCalcCpuLoadThreshold
...
VAR(uint32, RTM_VAR_NOINIT)                       Rtm_CpuLoadOverhead[RTM_NUMBER_OF_CORES];
...
VAR(uint8, RTM_VAR_NOINIT)                        Rtm_IsCpuLoadMeasurementPointResultOverflown[RTM_NUMBER_OF_CORES];
...
VAR(uint32, RTM_VAR_NOINIT)                       Rtm_CpuLoadOverhead;
...
RTM_LOCAL VAR(uint8, RTM_VAR_NOINIT)              Rtm_IsCpuLoadMeasurementPointResultOverflown; /* PRQA S 3408 */  /* MD_Rtm_3408 */
#   if defined (RTM_CPU_LOAD_AUTOSTART)
 ...
 *  \ingroup     cpuLoad
 *********************************************************************************************************************/
/* PRQA S 3408, 3447 5 */  /* MD_Rtm_3408 */
...
RTM_LOCAL_INLINE FUNC(void, RTM_CODE) Rtm_Init_CpuLoadMeasurement(void)
{
  ...
  Rtm_IsCpuLoadMeasurementPointResultOverflown = RTM_FALSE;
  ...
  Rtm_Hook_StartCalcCpuLoadThreshold(); /* PRQA S 3109 */  /* MD_MSR_14.3 */
  ...
  Rtm_Hook_StopCalcCpuLoadThreshold(); /* PRQA S 3109 */  /* MD_MSR_14.3 */
...
# if (RTM_MULTICORE_SUPPORT == STD_ON)
  {
    ...
    {
      Rtm_CpuLoadLastTicks[locCoreId] = 0u;
    }
  }
# else
  Rtm_CpuLoadLastTicks = 0u;
# endif
#endif
...
RTM_LOCAL_INLINE FUNC(uint8, RTM_CODE) Rtm_MeasCtr_ProceedLiveMeasurement(Rtm_MeasurementTimestampType CurrentTime)
{
  ...
  else
  {
    ...
    {
      Rtm_Time_GetCounterDelta(Rtm_LastMainFunctionTime, CurrentTime, &Rtm_AverageMainFunctionCycleTime, 0);
      Rtm_LastMainFunctionTime = CurrentTime;
    }
...
RTM_LOCAL_INLINE FUNC(uint8, RTM_CODE) Rtm_MeasCtr_ProceedSerialOrParallelMeasurement(Rtm_MeasurementTimestampType CurrentTime)
{
  ...
  if (Rtm_EndlessMeasurement == RTM_TRUE)
  {
    ...
    Rtm_Time_GetCounterDelta(Rtm_LastMainFunctionTime, CurrentTime, &currentMainFuncCycleTime, 0);
...
#   endif
      ...
      if (Rtm_CpuLoadLastTicks > Rtm_Results[RTM_CPU_LOAD_MEASUREMENT_ID].time)
      {
        ...
        Rtm_IsCpuLoadMeasurementPointResultOverflown = TRUE;
      }
...
#   endif
  ...
  if (EnterThread < RTM_NUMBER_OF_THREADS)
  {
    ...
    if (Rtm_ActiveMeasurementId < RTM_MEAS_SECTION_COUNT)
    {
      ...
      Rtm_Time_GetCounterDelta(Rtm_ThreadContext[EnterThread].preemptionTime, CurrentTime, &CurrentTime, Rtm_ActiveMeasurementId);
...
#  if (RTM_CTR_DIRECTION == RTM_UP)
  ...
  if ((EndValue) >= (StartValue))
  {
    ...
    {
      *ResultPtr = ((((uint32)Rtm_TimerOverrunCnt - Rtm_PrevOverrunCounterValues[MeasurementId]) * ((uint16)RTM_MEASUREMENT_MAX_VAL) ) + EndValue) - StartValue;
    }
    else
    {
      *ResultPtr = ((((uint32)((256 - Rtm_PrevOverrunCounterValues[MeasurementId]) + Rtm_TimerOverrunCnt)) * ((uint16)RTM_MEASUREMENT_MAX_VAL) ) + EndValue) - StartValue;
    }
  }
  ...
  else
  {
    if (Rtm_TimerOverrunCnt > Rtm_PrevOverrunCounterValues[MeasurementId])
    {
      *ResultPtr = ((((uint32)(Rtm_TimerOverrunCnt - Rtm_PrevOverrunCounterValues[MeasurementId])) * ((uint16)RTM_MEASUREMENT_MAX_VAL) ) - StartValue) + EndValue;
    }
    else
    {
      if (Rtm_TimerOverrunCnt == Rtm_PrevOverrunCounterValues[MeasurementId])
      {
        *ResultPtr = ((((uint32)1) * ((uint16)RTM_MEASUREMENT_MAX_VAL) ) - StartValue) + EndValue;
      }
      else
      {
        *ResultPtr = ((((uint32)((256 - Rtm_PrevOverrunCounterValues[MeasurementId]) + Rtm_TimerOverrunCnt)) * ((uint16)RTM_MEASUREMENT_MAX_VAL) ) - StartValue) + EndValue;
      }
    }
  }
#  else /* if (RTM_CTR_DIRECTION == RTM_DOWN) */
  ...
  else
  {
    ...
    else
    {
      if (Rtm_TimerOverrunCnt == Rtm_PrevOverrunCounterValues[MeasurementId])
      {
        *ResultPtr = (((uint32)(1) * RTM_MEASUREMENT_MAX_VAL) - EndValue) + StartValue;
      }
      else
      {
        *ResultPtr = ((((uint32)(256 - Rtm_PrevOverrunCounterValues[MeasurementId]) + Rtm_TimerOverrunCnt) * ((uint16)RTM_MEASUREMENT_MAX_VAL)) - EndValue) + StartValue;
...
#  endif /* (RTM_CTR_DIRECTION == RTM_UP) */
  ...
  RTM_DUMMY_STATEMENT(MeasurementId); /* PRQA S 3112, 3199 */ /* MD_MSR_14.2 */ /*lint -e{438} */
...
# endif
    {
      Rtm_Time_GetCurrentTime(&currentTime); /* PRQA S 3109 */  /* MD_MSR_14.3 */
...
#   if (RTM_MULTICORE_SUPPORT == STD_ON)
      /* ESCAN00078841 */
      Rtm_Time_GetCounterDelta(Rtm_CpuLoadStartTimestamp[locCoreId], currentTime, &cpuLoadSinceLastCall, RTM_CPU_LOAD_MEASUREMENT_ID);
...
#   else
      /* ESCAN00078841 */
      Rtm_Time_GetCounterDelta(Rtm_CpuLoadStartTimestamp, currentTime, &cpuLoadSinceLastCall, RTM_CPU_LOAD_MEASUREMENT_ID);
...
FUNC(void, RTM_CODE) Rtm_InitCpuLoadMeasurement(uint16 coreId, uint32 cpuMeasId)
{
  ...
  Rtm_IsCpuLoadMeasurementPointResultOverflown[coreId] = RTM_FALSE;
...
#  if (RTM_CPU_LOAD_CONTROL_MODE != RTM_CPU_LOAD_CONTROL_OFF)
          ...
          if (Rtm_IsCpuLoadMeasurementPointResultOverflown == RTM_TRUE)
          {
            Rtm_IsCpuLoadMeasurementPointResultOverflown = RTM_FALSE;
            errorCode = RTM_E_MEASUREMENT_POINT_RESULT_OVERFLOWN;
          }
...
#  if (RTM_CPU_LOAD_CONTROL_MODE != RTM_CPU_LOAD_CONTROL_OFF)
          if (Rtm_IsCpuLoadMeasurementPointResultOverflown[coreId] == RTM_TRUE)
          {
            Rtm_IsCpuLoadMeasurementPointResultOverflown[coreId] = RTM_FALSE;
            errorCode = RTM_E_MEASUREMENT_POINT_RESULT_OVERFLOWN;
          }

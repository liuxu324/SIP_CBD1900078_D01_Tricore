...
#if ( (UDPNM_SW_MAJOR_VERSION != 3u) || (UDPNM_SW_MINOR_VERSION != 3u) || (UDPNM_SW_PATCH_VERSION != 1u) )
...
# if ( UDPNM_ACTIVE_WAKEUP_BIT_ENABLED == STD_ON )
    if( UdpNm_VCfgGetActiveWakeupBitEnabled(UdpNmChannelIdx) == TRUE )
    {
      UdpNm_TxPdu_SetActiveWakeupBit(UdpNmChannelIdx);
    }
# endif
  }
...
# endif /* UDPNM_NODE_DETECTION_ENABLED */
  ...
  else if ( NM_STATE_READY_SLEEP == UdpNm_State[UdpNmChannelIdx] )
  {
    ...
    {      
      ...
      if( UdpNm_VCfgGetActiveWakeupBitEnabled(UdpNmChannelIdx) == TRUE )
      {
        UdpNm_TxPdu_ClearActiveWakeupBit(UdpNmChannelIdx);
      }
#endif
    }
  }
...
#endif /* UDPNM_COORDINATOR_SYNC_SUPPORT */
/*if car wakeup Rx enabled is configured*/
#if ( UDPNM_CAR_WAKEUP_RX_ENABLED == STD_ON )
    /* Check whether Car Wake Up Byte can be found inside RX PDU buffer */
    if ( ( UdpNm_VcfgGetCarWakeupRxEnabled(UdpNmChannelIdx) ) && ( PduInfoPtr->SduLength > UdpNm_VcfgGetCarWakeupBytePosition(UdpNmChannelIdx) ) )
    {
      /* Compare received CarWakeUp Byte against CarWakeUpBit mask */
      if( ( ( UdpNm_RxPduBuffers[UdpNmChannelIdx][UdpNm_VcfgGetCarWakeupBytePosition(UdpNmChannelIdx)] ) & ( UdpNm_VcfgGetCarWakeupBitPosition(UdpNmChannelIdx) ) ) == UdpNm_VcfgGetCarWakeupBitPosition(UdpNmChannelIdx) )
      {
#if ( UDPNM_CAR_WAKEUP_FILTER_ENABLED == STD_ON )
        /* If CarWakeUpFilterEnabled is there but is not configured || NID is present && the received Node ID is equal to "Car Wake Up Filter Node Id"*/
        if( ( !UdpNm_VcfgGetCarWakeupFilterEnabled(UdpNmChannelIdx) ) || 
            ( ( UdpNm_VCfgGetPduNidPosition(UdpNmChannelIdx) != UDPNM_PDU_OFF ) && 
              ( UdpNm_RxPduBuffers[UdpNmChannelIdx][UdpNm_VCfgGetPduNidPosition(UdpNmChannelIdx)] == UdpNm_VCfgGetCarWakeupFilterNodeId(UdpNmChannelIdx) )
            )
          )
#endif
        {
          Nm_CarWakeUpIndication(UdpNm_VCfgGetComMChannelHandle(UdpNmChannelIdx));
        }
      }
    }
#endif
...
} /* PRQA S 2006, 6050, 6010, 6030 */ /* MD_MSR_14.7, MD_MSR_STCAL, MD_MSR_STPTH, MD_MSR_STCYC */

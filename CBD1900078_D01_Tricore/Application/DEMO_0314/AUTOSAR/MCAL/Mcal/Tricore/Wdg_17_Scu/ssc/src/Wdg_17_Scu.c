...
#if ( WDG_17_SCU_AR_RELEASE_MINOR_VERSION != 2U )
...
#if ( WDG_17_SCU_SW_MAJOR_VERSION != 2U )
...
#if( (WDG_17_SCU_DISABLE_REJECT_DEM_REPORT == WDG_17_SCU_ENABLE_DEM_REPORT) ||\
(WDG_17_SCU_DISABLE_REJECT_DEM_REPORT == WDG_17_SCU_ENABLE_DEM_REPORT) )
...
/* WDGx initialization */
static void Wdg_lInit(uint32 CoreId);
...
static Std_ReturnType Wdg_lChangeMode(WdgIf_ModeType Mode, uint32 CoreId);
...
(WdgIf_ModeType Mode, uint32 CoreId);

/*Function to operate the Service timer in respective WDG mode */
static void Wdg_lStartOrStopTimeEngine(WdgIf_ModeType Mode, uint32 CoreId);

/* Function to calculate Timeout counter */
static void Wdg_lTimeoutHandler
(WdgIf_ModeType Mode, uint16 timeout, uint32 CoreId);
  ...
  if ( Wdg_17_Scu_Status[CoreId]->Wdg_17_Scu_DriverState == WDG_17_SCU_IDLE)
    ...
    if(ReturnValue == E_OK)
    {
      ...
      if(GtmChConfig->TimerType == MCU_GTM_TIMER_TOM)
      {
        ...
        De-initialize TOM Channel
        [/cover] */
        Mcu_17_Gtm_TomChannelDeInit(GtmModuleNumber, GtmChannelNumber);
      }
      else  /* ATOM channel is selected*/
      {
        ...
        De-initialize ATOM Channel
        [/cover] */
        Mcu_17_Gtm_AtomChannelDeInit(GtmModuleNumber, GtmChannelNumber);
      }
      #endif
...
(const WdgIf_ModeType Mode, const uint32 CoreId)
{
  ...
  switch(Mode)
  {
    ...
    case WDGIF_SLOW_MODE:
    {
      ...
      /* MISRA2012_RULE_13_2_JUSTIFICATION: No side effects foreseen.Even there
         is an over flow it is taken cared with compare register */
      /* MISRA2012_RULE_1_3_JUSTIFICATION: No side effects foreseen.
         This rule  violation is agreed as we need to store the address passed
         is in the called function in many scenarios.   */
      ...
      /* [cover parentID={B16EF3C4-D4E2-4b32-96E5-1173C27CFFBB}]
        ...
        with the tmeoutcount((ModeRefreshtime*100000)/STM timer Resolution)
        [/cover] */
      StmConfig.CompareRegVal = \
       (uint32)(Wdg_17_Scu_StmTim0[StmTimerNumber]->U) + TimeOutCount;
      /* MISRA2012_RULE_1_3_JUSTIFICATION: No side effects foreseen.
         This rule  violation is agreed as we need to store the address passed
         is in the called function in many scenarios.   */
    ...
    case WDGIF_FAST_MODE:
    {
      ...
      /* MISRA2012_RULE_13_2_JUSTIFICATION: No side effects foreseen.Even if
        there is an over flow it is taken care with compare register */
      /* MISRA2012_RULE_1_3_JUSTIFICATION: No side effects foreseen.
        This rule  violation is agreed as we need to store the address passed
        is in the called function in many scenarios.   */
...
**                                                                            **
** Syntax           : void Wdg_lInit(uint32 CoreId)                           **
**                                                                            **
      ...
      if(ErrorFlag == (Std_ReturnType)E_OK)
      {
        ...
        if(Wdg_17_Scu_Status[CoreId]->Wdg_TimeoutCounter > 0x00U)
        {
              ...
              WDGIF_SLOW_MODE)
          {
            ...
            TimeOutCount = \
            ((Wdg_ConfigPtr[CoreId]->SlowModeRefreshTime * WDT_TIMEOUT_FACTOR) / \
                            TimeOutResolution);
          }
          else
          {
            ...
            /* [cover parentID={1BB008EB-ECCD-4540-8F56-D8846DB93F1A}]
                 ...
                 and timer resolution [/cover] */
            TimeOutCount = \
            ((Wdg_ConfigPtr[CoreId]->FastModeRefreshTime * WDT_TIMEOUT_FACTOR) / \
                            TimeOutResolution);
          }
          ...
          /* MISRA2012_RULE_13_2_JUSTIFICATION: No side effects foreseen.Even if
              * there is an over flow it is taken care with compare register */
          /* MISRA2012_RULE_1_3_JUSTIFICATION: No side effects foreseen.
              * This rule  violation is agreed as we need to store the address
              * passed is in the called function in many scenarios. */
          ...
          StmConfig.CompareRegVal = \
            (uint32)(Wdg_17_Scu_StmTim0[StmTimerNumber]->U) + TimeOutCount;
          /* MISRA2012_RULE_1_3_JUSTIFICATION: No side effects foreseen.
              * This rule  violation is agreed as we need to store the
              * address passed is in the called function in many scenarios. */
